...
while True:
    ...
    if play:
        ...
        # Move the ball
        ball.setx(
            ball.xcor() + ball.dx)  # xcor and ycor is a function which displays your object coordinate in x or in y
        ...
        if ball.xcor() < -390:  # for X negative
            client1_send("right/" + str(ball.xcor()) + "/" + str(ball.ycor()) + "/" + str(ball.dx) + "/" + str(
                ball.dy) + '/' + str(score_a) + '/' + str(score_b), s)
            play = False
        ...
        if (ball.xcor() > 240 and ball.xcor() < 250) and (
                ball.ycor() < paddleLeft.ycor() + 40 and ball.ycor() > paddleLeft.ycor() - 40):
            ball.setx(240)
            ball.dx = ball.dx * -1
